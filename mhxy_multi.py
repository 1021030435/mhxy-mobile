...
def open_huodong(windows):
    ...
    click(list_window_region[windows][0] + int(list_window_region[windows][2] / 2), list_window_region[windows][1] + int(list_window_region[windows][3] / 2))  # 移到窗口中间，点击以激活窗口
    while is_start:
...
def do_action(windows):
    ...
    if get_rw("shimenButton",windows):  # 师门确认按钮
        ...
        flag = 1
        time.sleep(0.1)
    elif get_rw("goumai",windows) or get_rw("goumai1",windows) or get_rw("goumai2",windows) or get_rw("goumai3",windows):  # 购买物品
        # 同时还应该查看是否展示了 请选择物品 判断是否东西呗抢先购买了情况 然后关闭界面
        get_rw("guanbi",windows) or get_rw("guanbi_1",windows) or get_rw("guanbi_2",windows) or get_rw("guanbi_3",windows)
        flag = 1
        time.sleep(0.1)
    ...
    #     time.sleep(0.1)
        ...
        flag = 1
        time.sleep(0.1)
    elif get_rw("guanbi",windows) or get_rw("guanbi_1",windows) or get_rw("guanbi_2",windows) or get_rw("guanbi_3",windows):  # 关闭窗口
        flag = 1
        time.sleep(0.1)
